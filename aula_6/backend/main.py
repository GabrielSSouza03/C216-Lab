...
class Professor(BaseModel):
    ...
    sala_de_atendimento: str = Field(min_length=1)
...
class ProfessorAtualizar(BaseModel):
    ...
    sala_de_atendimento: Optional[str] = Field(default=None, min_length=1)


def row_to_professor(row):
    return {
        "id": row["id"],
        "nome": row["nome"],
        "email": row["email"],
...
async def listar_professores():
    ...
    try:
        ...
        return [row_to_professor(row) for row in rows]
    finally:
        await conn.close()
